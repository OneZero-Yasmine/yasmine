...
这是 Yasmine 的 Eleventy 静态网站，部署到 GitHub Pages 的 `/yasmine/` 子路径。开始工作前必须完整阅读 `项目.md`；详细规范以该文件为准。
...
- 所有公开页面、元数据、仓库介绍和文档只使用英文名 `Yasmine`；不得公开真名或其其他写法。
- 不得披露网站的内部用途、目标对象、申请意图或未公开的个人规划，即使这些信息存在于本地私有资料中。
